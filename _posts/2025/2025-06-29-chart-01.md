...
categories: [ compose,android-development ]
tags: [ android,compose,chart ]
math: true
---
...
앞서 원 한바퀴는 **2π 라디안**을 의미한다고 설명하였습니다. 그리고 정육각형에서 원점을 중심으로 각 꼭짓점까지 선분을 그으면  6개의 합동(동일한) 이등변 삼각형이 만들어집니다. 합동인 이등변삼각형들은 각자 동일한 영역을 차지하기 때문에 중심에 있는 각 또한 마찬가지로 2π 라디안을 6개의 동일한 부분으로 균등하게 나눈 것과 동일합니다. 따라서 정육각형 내부에 중심각은 π/3 라디안을 의미합니다. 이 공식을 모든 다각형에 대해 일반화 한다면 아래와 같이 정의됩니다.
